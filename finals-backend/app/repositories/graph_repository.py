...
# Words too common in internal documentation to carry any signal about duplication.
_STOPWORDS = frozenset("""
a an the and or of to in for on at by with from as is are was were be been it its this
that these those we our you your they their he she his her not no if then than so such
will would can could should may might must do does did have has had how what when where
which who whom why any all each other some more most into over under about after before
""".split())


# Longest-first so "reconciliation" reduces past "-ation" before "-ion" is considered.
_SUFFIXES = ("ational", "ization", "iveness", "ation", "ities", "ement", "ingly",
             "ions", "ing", "ies", "ity", "ive", "ers", "ed", "es", "er", "s")


def _stem(word: str) -> str:
    """Crude suffix stripper. Enough to make 'payments'/'payment' and
    'reconcile'/'reconciliation' collide, which is what duplicate detection needs."""
...
class GraphRepository:
    ...
    def _similarity_pairs(self, org_id: str) -> List[Tuple[str, str, float]]:
        """
        Duplicate detection without an embedding model.

        Titles are compared with the overlap coefficient rather than Jaccard, because a
        duplicate usually *extends* the original title rather than matching it word for
        word ("Payment Reconciliation SOP" vs "Duplicate: Payment Reconciliation SOP").
        Jaccard punishes the extra word; containment does not. Summaries are compared
        with Jaccard, where symmetric overlap is the honest measure. Only pairs at or
        above the classifier's near-duplicate floor are returned, so the scan stays cheap
        and the classifier keeps ownership of the thresholds.
        """
        docs = []
        for k in graph_store.nodes_by_label("Knowledge", org_id=org_id):
            title_tokens = _tokens(k.get("title") or "")
            body_tokens = _tokens(k.get("summary") or "")
            if title_tokens:
                docs.append((k["id"], title_tokens, body_tokens))
        ...
        for i in range(len(docs)):
            ...
            for j in range(i + 1, len(docs)):
                id_b, tb, bb = docs[j]
                score = 0.65 * containment(ta, tb) + 0.35 * jaccard(ba, bb)
                if score >= 0.75:
    ...
    async def build_disease_scan_inputs(self, org_id: str) -> Dict[str, Any]:
        ...
        - knowledge_cancer: real title/summary text similarity (see _similarity_pairs)
        """
        ...
        return {
            ...
            "similarity_pairs": self._similarity_pairs(org_id),

...
class KnowledgeRepository(BaseRepository[KnowledgeItem]):
    ...
    async def get_recent(
        ...
        `source_type` filters inside the query on purpose. Filtering the result of an
        already-limited query drops rows the caller asked for: with limit=3 and three
        policy documents at the top, a document-only caller got nothing back.
        """
        stmt = select(KnowledgeItem).where(KnowledgeItem.org_id == org_id)
        if source_type:
            stmt = stmt.where(KnowledgeItem.source_type == source_type)
        if dept_id:
            stmt = stmt.where(KnowledgeItem.dept_id == dept_id)
        if submitted_by_id:
            stmt = stmt.where(KnowledgeItem.submitted_by_id == submitted_by_id)
        result = await self.session.execute(
            stmt.order_by(KnowledgeItem.created_at.desc()).limit(limit)
        )

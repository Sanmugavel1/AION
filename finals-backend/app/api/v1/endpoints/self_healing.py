...
async def generate_recommendations(
    ...
    # An open recommendation for the same problem already on the board must not be
    # duplicated — re-running the scan should refresh the list, not stack copies of it.
    open_stmt = select(HealingAction).where(
        HealingAction.org_id == UUID(current_user.org_id),
        HealingAction.status.in_(("pending", "approved")),
    )
    existing = {
        (a.action_type, a.title) for a in (await db.execute(open_stmt)).scalars().all()
    }

    created, skipped = [], 0
    for rec in recommendations:
        key = (rec["action_type"], rec["title"])
        if key in existing:
            skipped += 1
            continue
        existing.add(key)
    ...
    await db.commit()
    return {"generated": len(created), "already_open": skipped, "recommendations": created}

...
class KnowledgeItem(BaseModel):
    ...
    # Status
    # Approval chain (SRS Sections 12-13). Existing rows default to "approved" so
    # knowledge seeded or ingested before the workflow existed stays visible.
    workflow_status: Mapped[str] = mapped_column(String(30), default="approved", nullable=False)
    submitted_by_id: Mapped[Optional[uuid.UUID]] = mapped_column(UUID(as_uuid=True), ForeignKey("users.id"))

...
# Absolute path to the project-root .env so it loads regardless of the process
# working directory (e.g. when uvicorn is launched with --app-dir from elsewhere).
...
class Settings(BaseSettings):
    ...
    # Data paths are written relative to the project ("./data/..."), which would otherwise
    # resolve against whatever directory the process happened to start in — launching from
    # elsewhere silently created a second, empty database. Anchor them to the project root
    # so the same data is used no matter where the app is started from.
    @field_validator("DATABASE_URL")
    @classmethod
    def _absolutise_sqlite_path(cls, v: str) -> str:
        prefix = "sqlite+aiosqlite:///"
        if v.startswith(prefix):
            raw = v[len(prefix):]
            if raw.startswith("./") or raw.startswith(".\\"):
                return prefix + (_PROJECT_ROOT / raw[2:]).as_posix()
        return v

    @field_validator("GRAPH_STORE_PATH")
    @classmethod
    def _absolutise_graph_path(cls, v: str) -> str:
        p = Path(v)

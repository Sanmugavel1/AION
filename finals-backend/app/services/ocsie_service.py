...
class OCSIEService:
    ...
    async def _detect_unfinished_work(self, user_id: str) -> Dict:
        """Work this person left mid-flight.

        Only two categories are real today, and both come from AION's own approval
        workflow: documents they submitted that are still awaiting a decision, and
        drafts they never submitted. The rest need the Section 38 integrations
        (GitHub, Slack, Jira, mail), which are not connected — they stay empty and
        are named in `pending_integrations` rather than being implied to work.
        """
        from sqlalchemy import select

        from app.models.enterprise import WorkflowStatus
        from app.models.knowledge import KnowledgeItem

        rows = (
            await self._db.execute(
                select(KnowledgeItem).where(
                    KnowledgeItem.submitted_by_id == user_id,
                    KnowledgeItem.workflow_status.in_(
                        list(WorkflowStatus.OPEN) + [WorkflowStatus.DRAFT]
                    ),
                )
            )
        ).scalars().all()
        ...
        def _brief(item) -> Dict:
            return {
                ...
                "since": item.created_at.isoformat() if item.created_at else None,
            }

        awaiting = [_brief(i) for i in rows if i.workflow_status in WorkflowStatus.OPEN]
        drafts = [_brief(i) for i in rows if i.workflow_status == WorkflowStatus.DRAFT]

        return {
            "pending_approvals": awaiting,
            "draft_documents": drafts,
            # Not measurable until the corresponding integration is connected.
            "open_pull_requests": [],
            "unanswered_emails": [],
            ...
            "incomplete_analyses": [],
            "blocked_workflows": [],
            "data_source": "AION approval workflow (documents awaiting a decision, and unsubmitted drafts)",
            "pending_integrations": ["GitHub", "Slack", "Jira", "Email"],
        }
